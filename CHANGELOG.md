...
### Real-REPL bug 4 partially closed (2026-05-03 transcript)

| Query | v4.38.0 | v4.38.5 |
|---|---|---|
| Қазақстанның барлық аймақтарын тізімдеңіз | "Аймақтарын туралы нақтырақ не білгіңіз келеді." (clarify, topic= mis-extracted «аймақтарын») | "**Аймақ туралы ең әуелі мынаны айтуға болады: Аймақ — аумақ.**" (topic correctly = «аймақ»; bridge fact surfaces) |
| Тұлғаларын / Жазушыларын / Аймақтарын (bare inflected forms) | clarify (treated as bare nouns) | resolved to root (тұлға / жазушы / аймақ) ✓ |
| Қазақстан / Сәлем / qty queries / confident facts | unchanged | unchanged ✓ |

### Innovations

**(1) 85 inflectional-duplicate entries purged from active lexicon** (80 from `segmentation_roots.json`, 5 from `apertium_imported_roots.json`). All entries are surface-forms of bare roots already in the lexicon, generated by Plural / P3 / case suffixes that the FST should derive dynamically. Conservative filter: only entries where stripping a strict suffix («-тарын / -терін / -ларын / -лерін / -дарын / -дерін» plus shorter variants down to «-тар / -тер / -лар / -лер / -дар / -дер») yields a base ≥ 4 chars that exists as a noun. Drops 14 ambiguous candidates with base < 4 chars (е.g. «көлдер» → «көл», «жерлерін» → «жер», «қаңтары» → «қаң») to avoid false positives on independent compounds and short-base inflections; those clusters are deferred to a follow-up review release.

**(2) FST gap fix in `synthesise_noun`**: when pronominal-н buffer is injected for P3 + Accusative, the regular `ACCUSATIVE` template is now skipped — in Kazakh, the pronominal-н IS the accusative marker after P3, so applying both produces wrong forms like «аймақтарынды» / «жасуларынды» (which then fail to match real surfaces in the analyse direction). Other case combinations (P3+Dat / P3+Loc / P3+Abl) preserved — pronominal-н is a true buffer there, with case suffix following.

**(3) Discovery: lexicon pollution masked FST gap.** Pre-v4.38.5, the polluted entries (e.g. `noun_aymaqtaryn` registered as bare root «аймақтарын») let analyse() match the entire surface as a single unanalysed noun, so the FST's broken P3+Acc derivation was never exercised in production. The purge exposes the gap; the synthesis fix closes it. Going forward, the absence of polluted entries is a forcing function: any future lexicon entry of shape «root + standard-suffix» MUST be derivable by the FST, or analysis fails — making FST coverage a hard, testable gate.

**(4) Recon script in commit message** (Python, 50 lines) — finds inflectional-duplicate candidates programmatically. Not committed as a binary because the cleanup itself is one-shot; the script is preserved in commit history for future similar passes (e.g. cleaning short-base candidates after FST gap fixes).
...
|---|---|
| Workspace tests | **865 passing** unchanged |
| Inflected-noun normalization | ✓ «Аймақтарын / Тұлғаларын / Жазушыларын» resolve to bare roots |
| Bug 4 partial closure | ✓ Topic now correctly = «аймақ»; bridge fact surfaces |
| Anti-regression — confident-fact controls | unchanged ✓ |
| Anti-regression — qty queries | unchanged ✓ |
| `cargo fmt --all --check` | clean |

### Latency / RSS (release-mode probes)

Unchanged from v4.38.0 (within noise; -85 lexicon entries is 0.5 % of total, no measurable impact).

### Deferred to v4.39.0+

- **Genitive-topic priority for list queries**: «Қазақстанның барлық аймақтарын тізімдеңіз» picks topic=аймақ (the head noun), but the ideal answer is the облыстар list stored under subject=қазақстан. Closing this requires a topic-marker preference for genitive nouns when query has list-intent — separate scope.
- **Short-base lexicon clusters** (көл / жер / көз / гүл): 14 candidates deferred. Need FST derivation verification for short stems before purging.
- **Lexicon function-word completion** (деп / осы / оның / деген / пен) — v4.39.0 milestone (79.48 % → 90 %+ morpheme coverage).

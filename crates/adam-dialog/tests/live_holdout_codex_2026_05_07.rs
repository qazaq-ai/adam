//! **v4.93.0 — Rust Book Codex 2026-05-07 audit live holdout (Codex 2026-05-07 audit fixes — memory poisoning + Rust routing) holdout.**
//!
//! Loads `data/eval/live_holdout_codex_2026_05_07.json` and asserts
//! every async-book-chapter-02 query (struct definition, fields, mutability,
//! tuple/unit struct variants, field init shorthand, struct update
//! syntax, data ownership, methods + &self/&mut self, automatic
//! referencing, associated functions + Self type, debug derive +
//! pretty-print + dbg!, multiple impl blocks, method/field
//! same-name) surfaces curriculum-grade content from the curated
//! `live REPL findings` entries plus deepened `rust_050…055`
//! (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new async-book-chapter-02 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_codex_2026_05_07.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_live_holdout_codex_2026_05_07 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_codex_2026_05_07: lexicon load failed")
}
...
#[test]
fn live_holdout_codex_2026_05_07() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_codex_2026_05_07: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_live_holdout_codex_2026_05_07 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "live_holdout_codex_2026_05_07: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

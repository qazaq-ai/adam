//! **v4.79.0 — Rust Book Chapter 1 holdout.**
//!
//! First per-chapter test under the directive «каждую главу
//! покрывай реальными тестами и сразу исправляй или корректируй
//! обучение». Loads `data/eval/rust_book_chapter_01_holdout.json`
//! and asserts every chapter-1 query (rustup / cargo / target /
//! println / fn main / edition / TOML / style) surfaces curriculum-
//! grade content from the curated `rust_200…rust_217` entries.
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new chapter-1 entries, append cases here; never edit
//! a captured set after it lands. Pattern repeats for each
//! `rust_book_chapter_NN.rs` going forward.
//!
...
const DATASET_PATH: &str = "../../data/eval/rust_book_chapter_01_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_book_chapter_01 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_book_chapter_01: lexicon load failed")
}
...
#[test]
fn rust_book_chapter_01_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_book_chapter_01: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_book_chapter_01 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "rust_book_chapter_01: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

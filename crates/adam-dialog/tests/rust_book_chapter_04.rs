//! **v4.80.5 — Rust Book Chapter 4 (Иелікті түсіну) holdout.**
//!
//! Loads `data/eval/rust_book_chapter_04_holdout.json` and asserts
//! every chapter-4 query (3 ownership rules, drop, stack vs heap,
//! String::from, move semantics, clone, Copy trait, function
//! ownership transfer, references, borrowing, dangling reference,
//! borrow rules 1+2, NLL, string slice, array slice, slice ranges,
//! data race) surfaces curriculum-grade content from the curated
//! `rust_254…270` entries plus deepened `rust_009/011/012/019/020`
//! (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new chapter-4 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/rust_book_chapter_04_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_book_chapter_04 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_book_chapter_04: lexicon load failed")
}
...
#[test]
fn rust_book_chapter_04_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_book_chapter_04: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_book_chapter_04 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "rust_book_chapter_04: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

//! **v5.4.5 — 64-turn audit live holdout.**
//!
//! Loads `data/eval/live_holdout_v5450_audit.json` and asserts that
//! the bare yes/no IsA query «<X> — <Y> (ме|ма|ба|бе|па|пе)?» is now
//! routed to a chain-aware confirm/deny answer instead of the
//! pre-v5.4.5 fallback behaviour (surface the most-central IsA fact
//! about <X>, ignoring <Y>). The bridge facts shipped in v5.4.5
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_v5450_audit.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "live_holdout_v5450_audit requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_v5450_audit: lexicon load failed")
}
...
#[test]
fn live_holdout_v5450_audit() {
    let lex = load_lexicon();
    let repo = load_repo();
    let runtime = load_runtime();
    if runtime.is_none() {
        eprintln!(
            "live_holdout_v5450_audit: runtime artefacts missing at \
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_v5450_audit: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("live_holdout_v5450_audit JSON must parse");
    ...
    eprintln!(
        "live_holdout_v5450_audit: {passed}/{total} cases passed across \
         {} categories",
        by_category.len()
    );
    ...
    assert_eq!(
        failures.len(),
        0,
        "live_holdout_v5450_audit: {} cases failed:\n{}",
        failures.len(),
        failures
            .iter()
    ...
    assert_eq!(
        passed, total,
        "live_holdout_v5450_audit: pass-rate floor is 100 %"
    );
}

//! **v4.89.5 — Rust Book Async Book Chapter 3 (async/await primer — state machine + lifetimes + async move) holdout.**
//!
//! Loads `data/eval/rust_async_book_chapter_03_holdout.json` and asserts
...
//! `rust_568…585` entries plus deepened `rust_050…055`
...
const DATASET_PATH: &str = "../../data/eval/rust_async_book_chapter_03_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_async_book_chapter_03 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_async_book_chapter_03: lexicon load failed")
}
...
#[test]
fn rust_async_book_chapter_03_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_async_book_chapter_03: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_async_book_chapter_03 JSON must parse");
    ...
    println!("\n=== rust_async_book_chapter_03_holdout ===");
    println!(
        "Overall: {} / {} passed ({:.1}%)",
        passed,
        total,
    ...
    assert_eq!(
        passed, total,
        "rust_async_book_chapter_03: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

//! **v4.88.5 — Rust Book Async Book Chapter 1 (Async бастамасы) holdout.**
//!
//! Loads `data/eval/rust_async_book_chapter_01_holdout.json` and asserts
//! every async-book-chapter-01 query (struct definition, fields, mutability,
...
//! `rust_532…549` entries plus deepened `rust_050…055`
//! (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new async-book-chapter-01 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/rust_async_book_chapter_01_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_async_book_chapter_01 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_async_book_chapter_01: lexicon load failed")
}
...
#[test]
fn rust_async_book_chapter_01_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_async_book_chapter_01: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_async_book_chapter_01 JSON must parse");
    ...
    println!("\n=== rust_async_book_chapter_01_holdout ===");
    println!(
        "Overall: {} / {} passed ({:.1}%)",
        passed,
        total,
    ...
    assert_eq!(
        passed, total,
        "rust_async_book_chapter_01: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

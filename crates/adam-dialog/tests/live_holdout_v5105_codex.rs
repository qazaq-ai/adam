//! **v5.10.5 — Codex follow-up review (B4.1) live holdout.**
//!
//! Proof-chain mode end-to-end. The user explicitly asks for a proof
//! («Қасқырдың тірі екенін дәлелде.» / «Дәлелде Қасқыр — тірі.»);
//! adam routes through the new `proof_chain.{with_data,empty}`
//! template family that performs the IsA chain step-by-step instead
//! of wrapping a yes/no verdict around it. Regression cases confirm
//! the existing yes/no IsA path and bare definition path still work.
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_v5105_codex.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "live_holdout_v5105_codex requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_v5105_codex: lexicon load failed")
}
...
fn live_holdout_v5105_codex() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_v5105_codex: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("live_holdout_v5105_codex JSON must parse");
    ...
    eprintln!(
        "live_holdout_v5105_codex: {passed}/{total} cases passed across {} categories",
        by_category.len()
    );
    ...
    assert_eq!(
        failures.len(),
        0,
        "live_holdout_v5105_codex: {} cases failed:\n{}",
        failures.len(),
        failures
            .iter()
    ...
    assert_eq!(
        passed, total,
        "live_holdout_v5105_codex: pass-rate floor is 100 %"
    );
}

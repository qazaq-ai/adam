//! **v4.80.0 — Rust Book Chapter 3 (Жалпы бағдарламалау ұғымдары) holdout.**
//!
//! Loads `data/eval/rust_book_chapter_03_holdout.json` and asserts
//! every chapter-3 query (immutable-by-default, const, integer
//! types + overflow + division, float, bool, char, tuple
//! destructuring, unit type, array repeat + OOB, fn return value,
//! block expression, if as expression, loop returning value, loop
//! labels, for over collection, comments) surfaces curriculum-grade
//! content from the curated `rust_237…253` entries plus deepened
//! `rust_022/038/039` (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new chapter-3 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/rust_book_chapter_03_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_book_chapter_03 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_book_chapter_03: lexicon load failed")
}
...
#[test]
fn rust_book_chapter_03_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_book_chapter_03: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_book_chapter_03 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "rust_book_chapter_03: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

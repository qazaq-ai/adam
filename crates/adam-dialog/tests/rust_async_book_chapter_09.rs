//! **v4.92.5 — Rust Book Async Book Chapter 9 (Final project — async HTTP server (capstone)) holdout.**
//!
//! Loads `data/eval/rust_async_book_chapter_09_holdout.json` and asserts
...
//! `rust_676…693` entries plus deepened `rust_050…055`
...
const DATASET_PATH: &str = "../../data/eval/rust_async_book_chapter_09_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_async_book_chapter_09 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_async_book_chapter_09: lexicon load failed")
}
...
#[test]
fn rust_async_book_chapter_09_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_async_book_chapter_09: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_async_book_chapter_09 JSON must parse");
    ...
    println!("\n=== rust_async_book_chapter_09_holdout ===");
    println!(
        "Overall: {} / {} passed ({:.1}%)",
        passed,
        total,
    ...
    assert_eq!(
        passed, total,
        "rust_async_book_chapter_09: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

//! **v5.11.0 — Codex follow-up review (B4.2) live holdout.**
//!
//! Countable propositions + epistemic refusal end-to-end. Closes
//! the second half of B4 — the generation arc atop the v5.9.0 G3.0
//! substrate.
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_v5110_codex.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "live_holdout_v5110_codex requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_v5110_codex: lexicon load failed")
}
...
fn live_holdout_v5110_codex() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_v5110_codex: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("live_holdout_v5110_codex JSON must parse");
    ...
    eprintln!(
        "live_holdout_v5110_codex: {passed}/{total} cases passed across {} categories",
        by_category.len()
    );
    ...
    assert_eq!(
        failures.len(),
        0,
        "live_holdout_v5110_codex: {} cases failed:\n{}",
        failures.len(),
        failures
            .iter()
    ...
    assert_eq!(
        passed, total,
        "live_holdout_v5110_codex: pass-rate floor is 100 %"
    );
}

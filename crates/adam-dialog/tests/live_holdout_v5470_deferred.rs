//! **v5.4.7 — deferred edge-case live holdout.**
//!
//! Loads `data/eval/live_holdout_v5470_deferred.json` and asserts that
//! the bare yes/no IsA query «<X> — <Y> (ме|ма|ба|бе|па|пе)?» is now
//! routed to a chain-aware confirm/deny answer instead of the
//! pre-v5.4.7 fallback behaviour (surface the most-central IsA fact
//! about <X>, ignoring <Y>). The bridge facts shipped in v5.4.7
//! (`data/world_core/{life,concept}_bridges.jsonl`) closed dead-end
//! abstract hubs (тіршілік иесі / тірі ағза / қазақ ханы → билеуші,
//! …) so transitive paths reach concrete predicates the user is
//! likely to ask about.
//!
//! Pass-rate floor: **100 %**. Each case is a real Kazakh phrasing
//! exercised end-to-end through `Conversation::turn`, with full
//! runtime artefacts (extracted facts + derived facts + morpheme
//! index) loaded from `data/retrieval/`. Append-only — when v5.5+
//! adds new bridge data, append cases here, never edit a captured
//! set after it lands.
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_v5470_deferred.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "live_holdout_v5470_deferred requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_v5470_deferred: lexicon load failed")
}
...
#[test]
fn live_holdout_v5470_deferred() {
    let lex = load_lexicon();
    let repo = load_repo();
    let runtime = load_runtime();
    if runtime.is_none() {
        eprintln!(
            "live_holdout_v5470_deferred: runtime artefacts missing at \
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_v5470_deferred: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("live_holdout_v5470_deferred JSON must parse");
    ...
    eprintln!(
        "live_holdout_v5470_deferred: {passed}/{total} cases passed across \
         {} categories",
        by_category.len()
    );
    ...
    assert_eq!(
        failures.len(),
        0,
        "live_holdout_v5470_deferred: {} cases failed:\n{}",
        failures.len(),
        failures
            .iter()
    ...
    assert_eq!(
        passed, total,
        "live_holdout_v5470_deferred: pass-rate floor is 100 %"
    );
}

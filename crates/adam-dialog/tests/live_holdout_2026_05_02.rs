//! **v4.30.0 — live holdout 2026-05-02.**
//!
//! Mirrors `live_holdout_2026_05_01.rs` but exercises the v4.30.0
//! patches: Rust extraction under `бағдарламалау тілі туралы`
//! qualifier, Раст Cyrillic-alias coverage, and adnominal-
//! demonstrative coreference (`бұл тілдегі` after a Rust topic).
//!
//! Adds support for **multi-turn cases** via the `context_turns`
//! field — a list of preceding queries that must be replayed
//! against the same `Conversation` instance before the test query
//! fires. Necessary for the demonstrative-coreference case which
//! only resolves correctly when prior turns established the
//! referent.
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_2026_05_02.json";
...
fn run_case(
    ...
    // Replay context turns first to establish dialog state. Only
    // the FINAL `query` is asserted; context turns are warm-up.
...
fn live_holdout_2026_05_02() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_2026_05_02: dataset must exist at {DATASET_PATH} — got {e}")
    });
    ...
    // v4.30.0 baseline: 100 % required. The dataset is small (5
    // cases) and all are direct regressions from yesterday's REPL
    // — every one must pass to ship v4.30.0.
    assert_eq!(
        passed, total,
        "live_holdout_2026_05_02 must pass all {} cases — failures above",
        total
    );
}

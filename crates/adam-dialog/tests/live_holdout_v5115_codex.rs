//! **v5.11.5 — Codex follow-up review (B5.1) live holdout.**
//!
//! Verifies that political-evaluative questions no longer misroute
//! through `StatementOfWellbeing`; they now route through the
//! political-recommendation refusal. Also confirms personal-wellbeing
//! and factual political questions are unaffected.
...
const DATASET_PATH: &str = "../../data/eval/live_holdout_v5115_codex.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "live_holdout_v5115_codex requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("live_holdout_v5115_codex: lexicon load failed")
}
...
fn live_holdout_v5115_codex() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("live_holdout_v5115_codex: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("live_holdout_v5115_codex JSON must parse");
    ...
    eprintln!(
        "live_holdout_v5115_codex: {passed}/{total} cases passed across {} categories",
        by_category.len()
    );
    ...
    assert_eq!(
        failures.len(),
        0,
        "live_holdout_v5115_codex: {} cases failed:\n{}",
        failures.len(),
        failures
            .iter()
    ...
    assert_eq!(
        passed, total,
        "live_holdout_v5115_codex: pass-rate floor is 100 %"
    );
}

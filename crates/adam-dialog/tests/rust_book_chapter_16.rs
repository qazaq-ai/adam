//! **v4.86.0 — Rust Book Chapter 16 (Қорқынышсыз қатарлас орындау) holdout.**
//!
//! Loads `data/eval/rust_book_chapter_16_holdout.json` and asserts
//! every chapter-16 query (struct definition, fields, mutability,
...
//! `rust_442…459` entries plus deepened `rust_050…055`
//! (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new chapter-16 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/rust_book_chapter_16_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_book_chapter_16 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_book_chapter_16: lexicon load failed")
}
...
#[test]
fn rust_book_chapter_16_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_book_chapter_16: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_book_chapter_16 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "rust_book_chapter_16: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

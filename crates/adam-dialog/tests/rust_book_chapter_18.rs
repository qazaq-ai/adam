//! **v4.87.0 — Rust Book Chapter 18 (Үлгілер мен сәйкестендіру) holdout.**
//!
//! Loads `data/eval/rust_book_chapter_18_holdout.json` and asserts
//! every chapter-18 query (struct definition, fields, mutability,
...
//! `rust_478…495` entries plus deepened `rust_050…055`
//! (replaced 1-line stubs).
//!
//! Per-chapter holdouts are append-only — when a future release
//! adds new chapter-18 entries, append cases here; never edit
...
const DATASET_PATH: &str = "../../data/eval/rust_book_chapter_18_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_book_chapter_18 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_book_chapter_18: lexicon load failed")
}
...
#[test]
fn rust_book_chapter_18_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_book_chapter_18: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_book_chapter_18 JSON must parse");
    ...
    assert_eq!(
        passed, total,
        "rust_book_chapter_18: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

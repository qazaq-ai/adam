//! **v4.91.0 — Rust Book Async Book Chapter 6 (Executing multiple futures — join! / try_join! / select! / FuturesUnordered) holdout.**
//!
//! Loads `data/eval/rust_async_book_chapter_06_holdout.json` and asserts
...
//! `rust_622…639` entries plus deepened `rust_050…055`
...
const DATASET_PATH: &str = "../../data/eval/rust_async_book_chapter_06_holdout.json";
...
fn load_lexicon() -> LexiconV1 {
    ...
    assert!(
        ...
        "rust_async_book_chapter_06 requires lexicon at {curated}"
    );
    LexiconV1::load(curated, apertium).expect("rust_async_book_chapter_06: lexicon load failed")
}
...
#[test]
fn rust_async_book_chapter_06_holdout() {
    ...
    let raw = std::fs::read_to_string(DATASET_PATH).unwrap_or_else(|e| {
        panic!("rust_async_book_chapter_06: dataset must exist at {DATASET_PATH} — got {e}")
    });
    let dataset: Dataset =
        serde_json::from_str(&raw).expect("rust_async_book_chapter_06 JSON must parse");
    ...
    println!("\n=== rust_async_book_chapter_06_holdout ===");
    println!(
        "Overall: {} / {} passed ({:.1}%)",
        passed,
        total,
    ...
    assert_eq!(
        passed, total,
        "rust_async_book_chapter_06: {} / {} passed (must be 100% — each case maps to a shipped curated entry)",
        passed, total
    );
}

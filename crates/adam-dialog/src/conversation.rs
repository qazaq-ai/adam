...
impl Conversation {
    ...
    pub fn turn_with_trace(
        ...
        let input: &str = {
            ...
            if is_bare_affirmation {
                if let Some(noun) = self.session.remove("pending_clarification_noun") {
                    // **Phase 27.A (2026-06-04 — rc14 audit)** —
                    // anti-loop guard.  When the user affirms a
                    // clarification («иә» after «Бәлкім, X туралы
                    // айтасыз ба»), we rewrite the input to «X
                    // туралы айтыңыз».  But if world_core doesn't
                    // have curated facts about X, the cascade
                    // produces the SAME clarification template
                    // again, and the next-turn would loop on the
                    // same prompt.  Stash the noun under a separate
                    // key so the end-of-turn post-process can
                    // detect the loop and rewrite the reply to
                    // «X туралы нақты дерегім жоқ».
                    self.session
                        .insert("just_consumed_clarification_noun".into(), noun.clone());
        ...
        // **Phase 27.A (2026-06-04 — rc14 audit)** — anti-loop guard.
        // If THIS turn was generated by consuming a pending
        // clarification («иә» rewrite at the top of the turn) AND
        // the cascade just re-emitted the SAME clarification template
        // («Бәлкім, X туралы…»), then world_core has no facts about
        // X and the next-turn affirmation would loop forever.
        // Override the output with an honest «X туралы нақты дерегім
        // жоқ» fallback so the conversation can move on.
        let final_output =
            if let Some(consumed_noun) = self.session.remove("just_consumed_clarification_noun") {
                if let Some(re_offered_noun) = extract_belkim_clarification_noun(&final_output)
                    && re_offered_noun.to_lowercase() == consumed_noun.to_lowercase()
                {
                    // Same noun re-offered after affirmation — break the loop.
                    let display_noun = {
                        let mut chars = consumed_noun.chars();
                        match chars.next() {
                            Some(c) => c.to_uppercase().collect::<String>() + chars.as_str(),
                            None => consumed_noun.clone(),
                        }
                    };
                    format!("{display_noun} туралы нақты дерегім жоқ.")
                } else {
                    final_output
                }
            } else {

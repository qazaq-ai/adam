...
pub(crate) const MULTIWORD_ENTITIES: &[&str] = &[
    ...
    "айнымалы тәсілі",
    "string әдісі",
    // **v4.86.5** — Rust Book chapter 17 (OOP features) deepening.
    // rust_460…477.
    "оо парадигмасы rust-та",
    "тұсаулау rust-та",
    "rust-та мұрагерлік жоқ",
    "көп пішінділік rust-та",
    "трейт-нысан",
    "dyn кілтсөзі",
    "vec<box<dyn trait>>",
    "статикалық vs динамикалық таратпа",
    "vtable",
    "нысан-қауіпсіз трейт",
    "clone нысан-қауіпсіз емес",
    "generic vs trait object таңдау",
    "күй үлгісі дәстүрлі",
    "күй үлгісі типтік жүйе арқылы",
    "default trait method идиомасы",
    "trait composition",
    "newtype pattern",
    "gui кітапхана үлгісі",
    // Objects.
    "rust философиясы",
    "оо тұжырымдамасы",
    "rust механизмі",
    "rust кілтсөзі",
    "trait object идиомасы",
    "rust шешімі",
    "trait ережесі",
    "trait шектеуі",
    "оо паттерні",
    ...
    "sync трейті",
    "send/sync auto-impl",
    "thread::spawn сигнатура",
    "data race prevention",
    // Objects.
    "rust принципі",
    "thread құрылымы",
    "thread идиомасы",
    "concurrency механизмі",
    "channel әдісі",
    "channel идиомасы",
    "concurrency примитиві",
    "mutex әдісі",
    "concurrency идиомасы",
    "concurrency трейті",
    "thread ережесі",
    "rust кепілдігі",
    // **v4.85.5** — Rust Book chapter 15 (smart pointers) deepening.
    // rust_424…441 + 3 stub deletions (085/086/088).
    "ақылды сілтеме",
    "box рекурсивті типке",
    "deref трейті",
    "deref coercion",
    "drop трейті",
    "std::mem::drop",
    ...
    "mock-нысан refcell",
    "rc<refcell> тіркесімі",
    "сілтеме циклы",
    "weak<t>",
    "rc::downgrade",
    "парент-бала ағаш",
    // Objects.
    "smart pointer паттерні",
    "compiler ыңғайлылығы",
    "rc әдісі",
    "concurrency айырмашылығы",
    "refcell әдісі",
    "refcell ережесі",
    "memory leak паттерні",
    "rust паттерні",
    "test паттерні",
    // **v4.85.0** — Rust Book chapter 14 (Cargo + Crates.io deeper) deepening.
    // rust_406…423.
    "cargo бейіндерін баптау",
    "opt-level",
    "cargo.toml метамәліметтер",
    "cargo doc",
    "/// vs //!",
    "doc бөлімдері",
    "doc-tests",
    ...
    "cargo yank",
    "cargo workspace",
    "workspace path тәуелділігі",
    "workspace cargo командалары",
    "cargo install",
    "cargo subcommand кеңейтуі",
    "crates.io экожүйесі",
    // Objects.
    "cargo баптауы",
    "cargo параметрі",
    ...
    "assert! макросы",
    "assert_eq",
    "partialeq debug талабы",
    "сынақ хабарламасы",
    "should_panic",
    "should_panic expected",
    "сынақта result",
    "параллельді сынақтар",
    "show-output жалаушасы",
    "сынақ фильтрі",
    "#[ignore]",
    "ignored жалаушасы",
    "бірлік тесттері",
    ...
    "test жалаушасы",
    "test түрі",
    // **v4.83.5** — Rust Book chapter 10 (generics + traits + lifetimes) deepening.
    // rust_351…368 + 2 stub replacements (014/073) + 3 rustadv deletions.
    "жалпылама тип параметрі",
    "структта жалпылама",
    "енамда жалпылама",
    "жалпылама әдіс",
    "мономорфтау",
    ...
    "трейт шектеуі",
    "where клаузасы",
    "impl trait қайтарушы",
    "шарт бойынша әдістер",
    "тіршілік мерзімі функцияда",
    "тіршілік мерзімі структта",
    "lifetime elision",
    "static лайфтайм",
    "generic трейт лайфтайм бірге",
    "қарыз тексергіш",
    // Objects.
    "generic шаблоны",
    "rust оптимизациясы",
    ...
    "код элементі",
    "сандық амал",
    // **v5.31.5** — military_kz domain. Compound entities introduced
    // by `data/world_core/military_kz.jsonl` (78 facts on Kazakh
    // armed forces statutory terminology, ranks, oath, conscription).
    // Sourced from Law 561-IV «On military service and the status of
    // military servicemen» and Decree 870/1992 (oath text). Every
    // compound subject/object that appears in those facts must be
    // mirrored here so `multiword_entity_hint` can detect it during
    // topic extraction.
    "армия генералы",
    "аға лейтенант",
    "аға офицер",
    ...
    "жоғары офицер",
    "жоғары сержанттық құрам",
    "запастағы азамат",
    "запастағы атақ",
    "запастағы лейтенант",
    "запастағы офицер",
    "запастағы сержант",
    "контрактілік әскери қызмет",
    "кіші офицер",
    "кіші сержант",
    "медицина запастағы офицер",
    "медицина қызметінің атағы",
    "мемлекеттік мекеме",
    "мемлекеттік функция",
    "мемлекеттік іс-шара",
    ...
    "отан қорғаушы күні",
    "отставкадағы атақ",
    "отставкадағы әскери қызметші",
    "офицерлер құрамы",
    "оқу бөлімшесі",
    "президент жарлығы",
    "президент жарлығымен бекітілген құжат",
    "ресми құжат",
    "салтанатты ант",
    "сержанттық құрам",
    "штаттық бірлік",
    ...
    "әскерге шақыруға дейінгі даярлық",
    "әскерге шақырылушы",
    "әскери айырым белгісі",
    "әскери ант",
    "әскери ант мәтіні",
    "әскери атақ",
    "әскери атақ түрі",
    ...
    "әскери бөлім",
    "әскери даярлық",
    "әскери даярлық оқушысы",
    "әскери есеп",
    "әскери жеңілдік",
    "әскери жиын",
    "әскери кафедра",
    "әскери кафедра студенті",
    "әскери киім нысаны",
    "әскери лауазым",
    "әскери міндетті",
    "әскери оқу орны",
    ...
    "әскери құрам санаты",
    "әскери ұйым",
    // Composite predicate-objects / state descriptors.
    "ант қабылдау міндеті",
    "халыққа адалдық",
    "халыққа адалдық міндеті",
    "конституцияны сақтау міндеті",
    "шағымдану құқығы",
    "қару беру құқығы",
    "ұрыс міндеті",
    "шақыру қағазы",
    ...
    "білім бойынша кейінге қалдыру",
    "денсаулық бойынша кейінге қалдыру",
    "бейбіт уақыт шақыруы",
    "жұмылдыру шақыруы",
    "ерекше киім",
    "ер азамат",
    // Numeric / duration tokens used as `object` in has_quantity facts.
    "бір жыл",
    "екі жыл",
    "үш жыл",
    "төрт жыл",
    ...
    "29 жасқа дейін",
    "32 жасқа дейін",
    "кемінде 2 жыл даярлық",
    "7 мамыр",
    // **v5.32.0** — `military_kz` batch 5: ВС РК structure from Law
    // 29-III «On defence and the Armed Forces». 22 facts about the
    // composition of the Armed Forces, leadership hierarchy (President
    // → Бас штаб → Defence Minister), special troops, reserve, other
    // military formations (National Guard).
    "агрессияға тойтарыс беру міндеті",
    "азаматтық персонал",
    "арнаулы операциялар күштері",
    "арнаулы әскерлер",
    "аумақтық тұтастық",
    "аумақтық әскерлер",
    "барлау әскерлері",
    ...
    "мемлекеттік шара",
    "мтқ әскерлері",
    "объект күзету міндеті",
    "орталық атқарушы орган",
    "оқу мекемесі",
    "президент лауазымы",
    "соғыс уақыты",
    "ғылыми мекеме",
    "қарулы күштер қызметкері",
    ...
    "әскери резерв",
    "әскери-саяси орган",
    "әскери-ғылыми мекеме",
    "әуе кеңістігі",
    // **v5.32.0** — `military_kz` batch 6: discipline + duties from
    // the 4 общевоинских уставов (Decree 364 of 05.07.2007). 20 facts
    // covering the 4 уставы themselves, military discipline, орders,
    // hierarchy, daily duty, garrison guard, awards / punishments,
    // drill / parade, leave / rest.

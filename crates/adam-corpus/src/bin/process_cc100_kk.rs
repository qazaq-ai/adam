// Streaming processor for CC-100 Kazakh.
//
// Input: stdin (one line per sentence; feed via `xzcat cc100_kk.txt.xz | ...`)
//
// Default mode (committed):
//   data/curated/cc100_kk_pack.json   — first 140 k samples (~49 MB)
//
// --full mode (local-only, v1.5.0+):
//   data/curated/cc100_kk_pack.json                   — shard 01
//   data/curated/shards/cc100_kk_shard_02_pack.json   — next 140 k
//   data/curated/shards/cc100_kk_shard_03_pack.json   — next 140 k
//   ... until EOF on the input stream.
//
//   `data/curated/shards/` is gitignored — shards are local-only for the
//   v2.0 retrieval engine. Committing everything would add hundreds of MB
//   of pack JSON to the repo for marginal validation value.
//
// v1.5.0 rewrite:
//   - shift from a 50 k cap to a 150 k first shard + optional --full sharding
//   - add the loanword-density filter (10 % cap, same as Wikipedia v1.3.0)
//   - unified surface with `process_wikipedia_kz` so the retrieval engine
//     can consume either shard set with the same schema
//
// Filters (rejection reasons, in order):
//   - length: word count outside [4, 40]
//   - latin:  any run of >= 3 consecutive ASCII letters
//   - cyrillic_ratio: < 80 % of alphabetic chars are Cyrillic
//   - url_email: line contains "http", "www.", "@", or ".com/.org/.net/.ru/.kz"
//   - repetitive: contains a 3-gram repeated >= 3 times
//   - punct_heavy: non-alphabetic char ratio > 30 %
//   - loanword_heavy: sample's loanword-token density > 10 %
//   - duplicate: lowercased text seen already

use std::{
    collections::HashSet,
    env,
    io::{self, BufRead},
    process::ExitCode,
};

use serde::Serialize;

const DEFAULT_OUTPUT: &str = "data/curated/cc100_kk_pack.json";
const SHARDS_DIR: &str = "data/curated/shards";

/// Samples per shard. CC-100 web-crawl sentences are longer on average
/// than Wikipedia article sentences, so 140 k (vs Wikipedia's 150 k) is
/// the sweet spot that keeps each pretty-printed shard under the 50 MB
/// GitHub warning threshold.
const SHARD_TARGET_SAMPLES: usize = 140_000;

const MIN_WORDS: usize = 4;
const MAX_WORDS: usize = 40;
const LOANWORD_DENSITY_CAP: f32 = 0.10;

/// Russian-only letters that never appear in native pre-modern Kazakh.
const RUSSIAN_ONLY: &[char] = &['ё', 'ф', 'ц', 'ч', 'щ', 'ъ', 'ь', 'э'];

/// Loanword suffix patterns shared across processors.
const LOANWORD_SUFFIXES: &[&str] = &[
    "ция",
    "логия",
    "графия",
    "тика",
    "изм",
    "ивный",
    "ильный",
    "альный",
    "альная",
    "альное",
    "ональный",
];

#[derive(Debug, Serialize)]
struct Sample {
    id: String,
    pack_name: String,
    source_id: String,
    domain: String,
    text: String,
}

#[derive(Debug, Serialize)]
struct Pack {
    version: String,
    name: String,
    target_language: String,
    script: String,
    source_license: String,
    source_url: String,
    attribution: String,
    lines_scanned: usize,
    sample_count: usize,
    skipped_length: usize,
    skipped_latin: usize,
    skipped_cyrillic_ratio: usize,
    skipped_url_email: usize,
    skipped_repetitive: usize,
    skipped_punct_heavy: usize,
    skipped_loanword_heavy: usize,
    skipped_duplicate: usize,
    samples: Vec<Sample>,
}

fn main() -> ExitCode {
    let args: Vec<String> = env::args().collect();
    let full_mode = args.iter().any(|a| a == "--full");

    let positional: Vec<&String> = args
        .iter()
        .skip(1)
        .filter(|a| !a.starts_with("--"))
        .collect();
    let first_shard_path = positional
        .first()
        .map(|s| s.to_string())
        .unwrap_or_else(|| DEFAULT_OUTPUT.to_string());
    let single_shard_cap: Option<usize> = positional.get(1).and_then(|s| s.parse().ok());

    let stdin = io::stdin();
    let reader = stdin.lock();

    let mut state = ShardState::new(first_shard_path, full_mode, single_shard_cap);
    let mut seen: HashSet<String> = HashSet::new();

    for line in reader.lines() {
        let Ok(line) = line else {
            continue;
        };
        state.add_line_scan();
        if state.is_finished() {
            break;
        }
        if state.lines_scanned().is_multiple_of(500_000) {
            eprintln!(
                "progress: scanned={} accepted={} shards_written={}",
                state.lines_scanned(),
                state.total_accepted(),
                state.shards_written(),
            );
        }
        let trimmed = line.trim();
        if trimmed.is_empty() {
            continue;
        }
        match accept(trimmed, &mut seen) {
            Ok(text) => state.accept(state.lines_scanned(), text),
            Err(reason) => state.add_skipped(reason),
        }
    }

    if let Err(e) = state.flush_final() {
        eprintln!("flush error: {e}");
        return ExitCode::FAILURE;
    }

    state.print_summary();
    ExitCode::SUCCESS
}

#[derive(Debug, Clone, Copy)]
enum SkipReason {
    Length,
    Latin,
    CyrRatio,
    Url,
    Repetitive,
    PunctHeavy,
    Loanword,
    Duplicate,
}

fn accept(line: &str, seen: &mut HashSet<String>) -> Result<String, SkipReason> {
    let word_count = line.split_whitespace().count();
    if !(MIN_WORDS..=MAX_WORDS).contains(&word_count) {
        return Err(SkipReason::Length);
    }
    if has_latin_run(line, 3) {
        return Err(SkipReason::Latin);
    }
    if cyrillic_alpha_ratio(line) < 0.80 {
        return Err(SkipReason::CyrRatio);
    }
    if contains_url_or_email(line) {
        return Err(SkipReason::Url);
    }
    if has_repetitive_trigram(line) {
        return Err(SkipReason::Repetitive);
    }
    if non_alpha_ratio(line) > 0.30 {
        return Err(SkipReason::PunctHeavy);
    }
    if loanword_density(line) > LOANWORD_DENSITY_CAP {
        return Err(SkipReason::Loanword);
    }
    let key = line.to_lowercase();
    if !seen.insert(key) {
        return Err(SkipReason::Duplicate);
    }
    Ok(line.to_string())
}

/// Streaming shard writer — mirrors `process_wikipedia_kz::ShardState`.
struct ShardState {
    full_mode: bool,
    single_shard_cap: Option<usize>,
    first_shard_path: String,

    shard_index: usize,
    samples: Vec<Sample>,

    lines_scanned: usize,
    total_accepted: usize,
    shards_written: usize,

    total_skipped_length: usize,
    total_skipped_latin: usize,
    total_skipped_cyr: usize,
    total_skipped_url: usize,
    total_skipped_repetitive: usize,
    total_skipped_punct: usize,
    total_skipped_loanword: usize,
    total_skipped_duplicate: usize,

    shard_lines: usize,
    shard_skipped_length: usize,
    shard_skipped_latin: usize,
    shard_skipped_cyr: usize,
    shard_skipped_url: usize,
    shard_skipped_repetitive: usize,
    shard_skipped_punct: usize,
    shard_skipped_loanword: usize,
    shard_skipped_duplicate: usize,

    finished: bool,
}

impl ShardState {
    fn new(first_shard_path: String, full_mode: bool, single_shard_cap: Option<usize>) -> Self {
        Self {
            full_mode,
            single_shard_cap,
            first_shard_path,
            shard_index: 1,
            samples: Vec::new(),
            lines_scanned: 0,
            total_accepted: 0,
            shards_written: 0,
            total_skipped_length: 0,
            total_skipped_latin: 0,
            total_skipped_cyr: 0,
            total_skipped_url: 0,
            total_skipped_repetitive: 0,
            total_skipped_punct: 0,
            total_skipped_loanword: 0,
            total_skipped_duplicate: 0,
            shard_lines: 0,
            shard_skipped_length: 0,
            shard_skipped_latin: 0,
            shard_skipped_cyr: 0,
            shard_skipped_url: 0,
            shard_skipped_repetitive: 0,
            shard_skipped_punct: 0,
            shard_skipped_loanword: 0,
            shard_skipped_duplicate: 0,
            finished: false,
        }
    }

    fn add_line_scan(&mut self) {
        self.lines_scanned += 1;
        self.shard_lines += 1;
    }

    fn lines_scanned(&self) -> usize {
        self.lines_scanned
    }

    fn total_accepted(&self) -> usize {
        self.total_accepted
    }

    fn shards_written(&self) -> usize {
        self.shards_written
    }

    fn add_skipped(&mut self, reason: SkipReason) {
        match reason {
            SkipReason::Length => {
                self.total_skipped_length += 1;
                self.shard_skipped_length += 1;
            }
            SkipReason::Latin => {
                self.total_skipped_latin += 1;
                self.shard_skipped_latin += 1;
            }
            SkipReason::CyrRatio => {
                self.total_skipped_cyr += 1;
                self.shard_skipped_cyr += 1;
            }
            SkipReason::Url => {
                self.total_skipped_url += 1;
                self.shard_skipped_url += 1;
            }
            SkipReason::Repetitive => {
                self.total_skipped_repetitive += 1;
                self.shard_skipped_repetitive += 1;
            }
            SkipReason::PunctHeavy => {
                self.total_skipped_punct += 1;
                self.shard_skipped_punct += 1;
            }
            SkipReason::Loanword => {
                self.total_skipped_loanword += 1;
                self.shard_skipped_loanword += 1;
            }
            SkipReason::Duplicate => {
                self.total_skipped_duplicate += 1;
                self.shard_skipped_duplicate += 1;
            }
        }
    }

    fn accept(&mut self, source_line: usize, text: String) {
        self.total_accepted += 1;
        let idx = self.total_accepted;
        self.samples.push(Sample {
            id: format!("cc100_kk_{idx:07}"),
            pack_name: "adam-cc100-kk-pack".to_string(),
            source_id: format!("cc100_kk_line_{source_line}"),
            domain: "web_crawl".to_string(),
            text,
        });

        if self.samples.len() >= SHARD_TARGET_SAMPLES {
            if let Err(e) = self.flush_current_shard() {
                eprintln!("flush error on shard {}: {e}", self.shard_index);
                std::process::exit(1);
            }
            if !self.full_mode {
                self.finished = true;
            }
        }

        if let Some(cap) = self.single_shard_cap {
            if self.samples.len() >= cap && self.shard_index == 1 {
                self.finished = true;
            }
        }
    }

    fn is_finished(&self) -> bool {
        self.finished
    }

    fn flush_final(&mut self) -> std::io::Result<()> {
        if !self.samples.is_empty() {
            self.flush_current_shard()?;
        }
        Ok(())
    }

    fn flush_current_shard(&mut self) -> std::io::Result<()> {
        let path = self.current_shard_path();
        let pack = Pack {
            version: env!("CARGO_PKG_VERSION").to_string(),
            name: format!("adam-cc100-kk-pack-shard-{:02}", self.shard_index),
            target_language: "kazakh".to_string(),
            script: "cyrillic".to_string(),
            source_license: "Common Crawl terms of use".to_string(),
            source_url: "https://data.statmt.org/cc-100/kk.txt.xz".to_string(),
            attribution: "Conneau et al. 2020, Unsupervised Cross-lingual Representation Learning at Scale (XLM-R)".to_string(),
            lines_scanned: self.shard_lines,
            sample_count: self.samples.len(),
            skipped_length: self.shard_skipped_length,
            skipped_latin: self.shard_skipped_latin,
            skipped_cyrillic_ratio: self.shard_skipped_cyr,
            skipped_url_email: self.shard_skipped_url,
            skipped_repetitive: self.shard_skipped_repetitive,
            skipped_punct_heavy: self.shard_skipped_punct,
            skipped_loanword_heavy: self.shard_skipped_loanword,
            skipped_duplicate: self.shard_skipped_duplicate,
            samples: std::mem::take(&mut self.samples),
        };

        if let Some(parent) = std::path::Path::new(&path).parent() {
            if !parent.as_os_str().is_empty() && !parent.exists() {
                std::fs::create_dir_all(parent)?;
            }
        }
        let json = serde_json::to_string_pretty(&pack).map_err(std::io::Error::other)?;
        std::fs::write(&path, json)?;
        eprintln!(
            "shard {:02}: wrote {} samples to {path}",
            self.shard_index, pack.sample_count
        );

        self.shards_written += 1;
        self.shard_index += 1;
        self.shard_lines = 0;
        self.shard_skipped_length = 0;
        self.shard_skipped_latin = 0;
        self.shard_skipped_cyr = 0;
        self.shard_skipped_url = 0;
        self.shard_skipped_repetitive = 0;
        self.shard_skipped_punct = 0;
        self.shard_skipped_loanword = 0;
        self.shard_skipped_duplicate = 0;
        Ok(())
    }

    fn current_shard_path(&self) -> String {
        if self.shard_index == 1 {
            self.first_shard_path.clone()
        } else {
            format!(
                "{SHARDS_DIR}/cc100_kk_shard_{:02}_pack.json",
                self.shard_index
            )
        }
    }

    fn print_summary(&self) {
        eprintln!(
            "TOTAL lines={} accepted={} skipped_length={} skipped_latin={} skipped_cyr={} skipped_url={} skipped_rep={} skipped_punct={} skipped_loanword={} skipped_dup={} shards_written={}",
            self.lines_scanned,
            self.total_accepted,
            self.total_skipped_length,
            self.total_skipped_latin,
            self.total_skipped_cyr,
            self.total_skipped_url,
            self.total_skipped_repetitive,
            self.total_skipped_punct,
            self.total_skipped_loanword,
            self.total_skipped_duplicate,
            self.shards_written,
        );
    }
}

fn has_latin_run(s: &str, min_run: usize) -> bool {
    let mut run = 0usize;
    for c in s.chars() {
        if c.is_ascii_alphabetic() {
            run += 1;
            if run >= min_run {
                return true;
            }
        } else {
            run = 0;
        }
    }
    false
}

fn cyrillic_alpha_ratio(s: &str) -> f32 {
    let mut alpha = 0usize;
    let mut cyr = 0usize;
    for c in s.chars() {
        if c.is_alphabetic() {
            alpha += 1;
            if ('\u{0400}'..='\u{04FF}').contains(&c) {
                cyr += 1;
            }
        }
    }
    if alpha == 0 {
        return 0.0;
    }
    cyr as f32 / alpha as f32
}

fn contains_url_or_email(s: &str) -> bool {
    let lower = s.to_ascii_lowercase();
    lower.contains("http")
        || lower.contains("www.")
        || lower.contains('@')
        || lower.contains(".com")
        || lower.contains(".org")
        || lower.contains(".net")
        || lower.contains(".ru")
        || lower.contains(".kz")
}

fn has_repetitive_trigram(s: &str) -> bool {
    let words: Vec<&str> = s.split_whitespace().collect();
    if words.len() < 9 {
        return false;
    }
    let mut counts: std::collections::HashMap<(&str, &str, &str), usize> =
        std::collections::HashMap::new();
    for w in words.windows(3) {
        let key = (w[0], w[1], w[2]);
        let c = counts.entry(key).or_insert(0);
        *c += 1;
        if *c >= 3 {
            return true;
        }
    }
    false
}

fn non_alpha_ratio(s: &str) -> f32 {
    let mut total = 0usize;
    let mut non_alpha = 0usize;
    for c in s.chars() {
        if c.is_whitespace() {
            continue;
        }
        total += 1;
        if !c.is_alphabetic() {
            non_alpha += 1;
        }
    }
    if total == 0 {
        return 0.0;
    }
    non_alpha as f32 / total as f32
}

fn loanword_density(text: &str) -> f32 {
    let words: Vec<&str> = text.split_whitespace().collect();
    if words.is_empty() {
        return 0.0;
    }
    let flagged = words.iter().filter(|w| is_loanword_token(w)).count();
    flagged as f32 / words.len() as f32
}

fn is_loanword_token(word: &str) -> bool {
    let cleaned: String = word
        .chars()
        .filter(|c| c.is_alphabetic() || *c == '-')
        .collect::<String>()
        .to_lowercase();
    if cleaned.chars().any(|c| RUSSIAN_ONLY.contains(&c)) {
        return true;
    }
    LOANWORD_SUFFIXES
        .iter()
        .any(|s| cleaned.ends_with(s) && cleaned.chars().count() > s.chars().count() + 1)
}

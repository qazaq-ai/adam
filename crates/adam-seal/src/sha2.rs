//! SHA-256 and SHA-512 (FIPS 180-4), pure Rust, no dependencies.
//!
//! SHA-512 is the internal hash of Ed25519 (RFC 8032); SHA-256 is the
//! default digest over the canonical protocol envelope.  Both are gated
//! on the FIPS / RFC known-answer vectors in `tests/`.
...
/// SHA-256 of `msg`, returning the 32-byte digest.
pub fn sha256(msg: &[u8]) -> [u8; 32] {
    let mut h = H256;
    let bit_len = (msg.len() as u64).wrapping_mul(8);

    // Pad: 0x80, then zeros, then 64-bit big-endian bit length -> multiple of 64.

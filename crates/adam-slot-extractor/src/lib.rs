...
/// On-disk Rung-A artefact. Same sparse layout as the E1 model
/// — only non-zero buckets are stored; the runtime dense buffer
/// is rebuilt on load via [`Artefact::densify`]. Schema version
/// `0.0.1` produced by `tools/intent_dataset/src/slot_train.rs`.
#[derive(Debug, Clone, Serialize, Deserialize)]
struct Artefact {
    schema_version: String,
    bucket_count: usize,
    labels: Vec<String>,
    /// Sparse weights — `(bucket_index, [per_class_weights])`.
    /// Inner vec length = `labels.len()`.
    #[serde(default)]
    sparse_weights: Vec<(u32, Vec<f32>)>,
    /// Runtime-only dense weight buffer.
    #[serde(skip)]
    dense: Vec<f32>,
}
...
impl Artefact {
    ...
    fn predict_tags(&self, tokens: &[String]) -> Vec<&str> {
        let n_classes = self.labels.len();
        let mut out: Vec<&str> = Vec::with_capacity(tokens.len());
        for i in 0..tokens.len() {
            let mut scores = vec![0.0_f32; n_classes];
            for (bucket, value) in features::extract(tokens, i, self.bucket_count) {
                let base = (bucket as usize) * n_classes;
                for (cls, score) in scores.iter_mut().enumerate() {
                    *score += value * self.dense[base + cls];
                }
            }
            // Argmax in log-domain (no need for softmax since
            // we only care about the winning index).
            let (best_idx, _) = scores
                .iter()
                .enumerate()
                .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
                .unwrap_or((0, &0.0));
            out.push(self.labels[best_idx].as_str());
        }
        out
    }
}
...
impl SlotExtractor {
    ...
    /// Rebuilds the runtime dense buffer from the sparse on-disk
    /// layout.
    ...
    ///
    /// **Closed-set invariant**: every emitted tag is one of the
    /// 11 BIO labels in [`BioTag`] — hallucination is
    /// structurally impossible.
    pub fn extract(&self, tokens: &[&str]) -> Result<Extraction, SlotExtractorError> {
        let artefact = self
            .artefact
            .as_ref()
            .ok_or(SlotExtractorError::NotLoaded)?;
        let tokens_owned: Vec<String> = tokens.iter().map(|s| s.to_string()).collect();
        let predicted = artefact.predict_tags(&tokens_owned);
        // Map string labels to typed BioTag; unknown labels map
        // to `O` (defensive — shouldn't fire because the labels
        // were emitted by the trainer which uses the same enum).
        let tags: Vec<BioTag> = predicted
            .iter()
            .map(|t| BioTag::parse(t).unwrap_or(BioTag::O))
            .collect();
        // Decode spans from BIO tags.
        let mut spans: Vec<Span> = Vec::new();
        let mut i = 0;
        while i < tags.len() {
            if tags[i].is_begin() {
                let slot = match tags[i].slot() {
                    Some(s) => s,
                    None => {
                        i += 1;
                        continue;
                ...
                while i < tags.len()
                    && matches!(
                        (tags[i], slot),
                        (BioTag::IPer, SlotType::Person)
                            | (BioTag::ILoc, SlotType::Location)
                            | (BioTag::IAge, SlotType::Age)
                            | (BioTag::IOcc, SlotType::Occupation)
                            | (BioTag::IFam, SlotType::Family)
                    )
                {
                    i += 1;
                }
                ...
                spans.push(Span {
                    ...
                    text,
                    // Score 1.0 is a placeholder — Rung A doesn't
                    // surface calibrated marginals. Future rungs
                    // (CRF / softmax) can fill this with real
                    // confidence.
                    score: 1.0,
                });
            } else {

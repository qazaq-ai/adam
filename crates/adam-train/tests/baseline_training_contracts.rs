...
use adam_train::{
    ...
    MiniCleanTrainingManifest, TinyCleanTrainingDomainPack, TinyCleanTrainingMissAuditDeltaReport,
    ...
    assemble_mini_clean_training_pack, assemble_tiny_clean_training_pack_from_corpus,
    ...
    build_mini_clean_training_miss_audit_report, build_mini_clean_training_report,
...
#[test]
fn mini_clean_training_pack_matches_manifest_assembly() {
    let manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/mini_clean_training_manifest.json"
    );
    let manifest: MiniCleanTrainingManifest =
        serde_json::from_str(&fs::read_to_string(manifest_path).expect("mini manifest file"))
            .expect("valid mini manifest json");
    let clean_manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/clean_training_corpus_manifest.json"
    );
    ...
    let expected: TinyCleanTrainingPack =
        serde_json::from_str(&fs::read_to_string(expected_path).expect("mini clean pack file"))
            .expect("valid mini clean pack json");

    let actual =
        assemble_mini_clean_training_pack(&manifest, &clean_manifest, &clean_pack).expect("pack");

    for sample_texts in manifest.preferred_sample_texts_by_domain.values() {
        for sample_text in sample_texts {
            assert!(
                actual
                    .samples
                    .iter()
                    .any(|sample| &sample.text == sample_text),
                "mini pack should include preferred sample `{sample_text}`"
            );
        }
    }
...
#[test]
fn mini_clean_training_pack_can_include_preferred_late_clean_sample() {
    let manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/mini_clean_training_manifest.json"
    );
    let mut manifest: MiniCleanTrainingManifest =
        serde_json::from_str(&fs::read_to_string(manifest_path).expect("mini manifest file"))
            .expect("valid mini manifest json");
    manifest.preferred_sample_texts_by_domain.insert(
        "education".to_string(),
        vec!["Оқу мазмұны терминнің қызметін қысқа түрде түсіндіреді.".to_string()],
    );
    let clean_manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/clean_training_corpus_manifest.json"
    ...
    assert!(actual.samples.iter().any(|sample| {
        sample.domain == "education"
            && sample.text == "Оқу мазмұны терминнің қызметін қысқа түрде түсіндіреді."
    }));
}

#[test]
fn mini_clean_training_report_matches_expected_regression_artifact() {
    let manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/baseline_training_manifest.json"
    );
    ...
    let acceptance_report: SourceAcceptanceReport = serde_json::from_str(
        &fs::read_to_string(acceptance_report_path).expect("acceptance report file"),
    )
    .expect("valid acceptance report json");
    let mini_manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/mini_clean_training_manifest.json"
    );
    let mini_manifest: MiniCleanTrainingManifest =
        serde_json::from_str(&fs::read_to_string(mini_manifest_path).expect("mini manifest file"))
            .expect("valid mini manifest json");
    let clean_manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/clean_training_corpus_manifest.json"
    ...
    let clean_pack: CleanTrainingCorpusPack =
        serde_json::from_str(&fs::read_to_string(clean_pack_path).expect("clean corpus pack file"))
            .expect("valid clean corpus pack json");
    let pack = assemble_mini_clean_training_pack(&mini_manifest, &clean_manifest, &clean_pack)
        .expect("mini pack");
    let expected_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
    ...
    let expected: TinyCleanTrainingReport =
        serde_json::from_str(&fs::read_to_string(expected_path).expect("mini training report"))
            .expect("valid mini training report json");

    let actual =
        build_mini_clean_training_report(&manifest, &registry, &rules, &acceptance_report, &pack)
            .expect("mini clean training report");
...
#[test]
fn mini_clean_training_miss_audit_report_matches_expected_regression_artifact() {
    let manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/baseline_training_manifest.json"
    );
    ...
    let expected: TinyCleanTrainingMissAuditReport = serde_json::from_str(
        &fs::read_to_string(expected_path).expect("mini training miss audit report"),
    )
    .expect("valid mini training miss audit report json");

    let actual = build_mini_clean_training_miss_audit_report(
        &manifest,
        &registry,
        &rules,
        &acceptance_report,
    ...
    .expect("mini clean training miss audit report");
...
fn tiny_clean_training_profile_promotion_report_matches_expected_regression_artifact() {
    ...
    let matrix_policy_report_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
    ...
    let matrix_policy_report: TinyCleanTrainingProfileExperimentMatrixPolicyReport =
        serde_json::from_str(
            &fs::read_to_string(matrix_policy_report_path)
                .expect("tiny profile experiment matrix policy report"),
        )
        .expect("valid tiny profile experiment matrix policy report json");
    let expected_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_promotion_report.json"
    );
    ...
    let actual =
        build_tiny_clean_training_profile_promotion_report(&manifest, &matrix_policy_report)
...
#[test]
fn tiny_clean_training_profile_experiment_matrix_policy_report_matches_expected_regression_artifact()
 {
    let manifest_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/curated/tiny_clean_training_profile_experiment_matrix_policy_manifest.json"
    );
    let manifest: TinyCleanTrainingProfileExperimentMatrixPolicyManifest = serde_json::from_str(
        &fs::read_to_string(manifest_path).expect("tiny profile experiment matrix policy manifest"),
    )
    .expect("valid tiny profile experiment matrix policy manifest json");
    let matrix_report_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_experiment_matrix_report.json"
    );
    ...
    let expected: TinyCleanTrainingProfileExperimentMatrixPolicyReport = serde_json::from_str(
        &fs::read_to_string(expected_path).expect("tiny profile experiment matrix policy report"),
    )
    .expect("valid tiny profile experiment matrix policy report json");

    let actual = build_tiny_clean_training_profile_experiment_matrix_policy_report(
        &manifest,
        &matrix_report,
    )
    .expect("tiny profile experiment matrix policy report");
    ...
    let selected_balanced = actual
        .candidate_decisions
        .iter()
        .find(|entry| entry.profile == "balanced")
        .expect("balanced candidate decision");
    assert!(selected_balanced.is_eligible);
    assert!(selected_balanced.rejection_reasons.is_empty());

    assert_eq!(actual, expected);
}

#[test]
fn tiny_clean_training_profile_experiment_matrix_policy_delta_report_matches_expected_regression_artifact()
 {
    let policy_report_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_experiment_matrix_policy_report.json"
    );
    let policy_report: TinyCleanTrainingProfileExperimentMatrixPolicyReport = serde_json::from_str(
        &fs::read_to_string(policy_report_path)
            .expect("tiny profile experiment matrix policy report"),
    )
    .expect("valid tiny profile experiment matrix policy report json");
    let expected_path = concat!(
    ...
    let expected: TinyCleanTrainingProfileExperimentMatrixPolicyDeltaReport = serde_json::from_str(
        &fs::read_to_string(expected_path)
            .expect("tiny profile experiment matrix policy delta report"),
    )
    .expect("valid tiny profile experiment matrix policy delta report json");

    let actual = build_tiny_clean_training_profile_experiment_matrix_policy_delta_report(
        &policy_report,
        &policy_report,
    );
...
fn foundation_overview_report_matches_expected_regression_artifact() {
    ...
    let tiny_profile_experiment_matrix_delta: TinyCleanTrainingProfileExperimentMatrixDeltaReport =
        ...
        .expect("valid tiny profile experiment matrix delta report json");
    let tiny_profile_experiment_matrix_policy_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_experiment_matrix_policy_report.json"
    );
    let tiny_profile_experiment_matrix_policy: TinyCleanTrainingProfileExperimentMatrixPolicyReport =
        serde_json::from_str(
            &fs::read_to_string(tiny_profile_experiment_matrix_policy_path)
                .expect("tiny profile experiment matrix policy report"),
        )
        .expect("valid tiny profile experiment matrix policy report json");
    let tiny_profile_experiment_matrix_policy_delta_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_experiment_matrix_policy_delta_report.json"
    );
    let tiny_profile_experiment_matrix_policy_delta:
        TinyCleanTrainingProfileExperimentMatrixPolicyDeltaReport = serde_json::from_str(
            &fs::read_to_string(tiny_profile_experiment_matrix_policy_delta_path)
                .expect("tiny profile experiment matrix policy delta report"),
        )
        .expect("valid tiny profile experiment matrix policy delta report json");
    ...
    let actual = build_foundation_overview_report(
        ...
        &tiny_profile_experiment_matrix_policy_delta,
...
fn foundation_overview_policy_readiness_tracks_substantive_policy_checks() {
    ...
    let matrix_policy_report: TinyCleanTrainingProfileExperimentMatrixPolicyReport =
        ...
        .expect("valid tiny profile experiment matrix policy report json");
    let matrix_policy_delta_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../data/training/tiny_clean_training_profile_experiment_matrix_policy_delta_report.json"
    );
    let matrix_policy_delta: TinyCleanTrainingProfileExperimentMatrixPolicyDeltaReport =
        serde_json::from_str(
            &fs::read_to_string(matrix_policy_delta_path)
                .expect("tiny profile experiment matrix policy delta report"),
        )
        .expect("valid tiny profile experiment matrix policy delta report json");

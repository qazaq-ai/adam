...
use adam_train::{
    ...
    MiniCleanTrainingManifest, assemble_mini_clean_training_pack, build_mini_clean_training_report,
};

const USAGE: &str = "usage: mini_train <training-manifest> <mini-manifest> <clean-corpus-manifest> <clean-corpus-pack>";
...
fn main() -> ExitCode {
    ...
    match build_mini_clean_training_report(&manifest, &registry, &rules, &acceptance_report, &pack)
    {
        Ok(report) => {
            println!(
                "{}",
                serde_json::to_string_pretty(&report)
                    .expect("mini clean training report serializes")
            );
            ExitCode::SUCCESS
        }
        Err(error) => {
            eprintln!("failed to build mini clean training report: {error}");
            ExitCode::FAILURE
        }
    }
}

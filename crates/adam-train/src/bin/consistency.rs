...
use adam_train::{BaselineTrainingManifest, build_baseline_training_consistency_report};

fn main() -> ExitCode {
    let mut args = env::args().skip(1);
    let Some(manifest_path) = args.next() else {
        eprintln!("usage: consistency <training-manifest>");
    ...
    match build_baseline_training_consistency_report(
        &manifest,
        &corpus,
        &registry,
        &rules,
        ...
        Ok(report) => {
            println!(
                "{}",
                serde_json::to_string_pretty(&report).expect("consistency report serializes")
            );
            ExitCode::SUCCESS
        }
        Err(error) => {
            eprintln!("failed to build baseline training consistency report: {error}");
            ExitCode::FAILURE
        }
    }
}

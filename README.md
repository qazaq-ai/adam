...
<p align="center">
  <a href="https://github.com/qazaq-ai/adam/releases"><img src="https://img.shields.io/badge/version-4.4.10-2EA44F?style=for-the-badge" alt="version"></a>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-687%20passing-2EA44F?style=flat-square" alt="tests">
  <img src="https://img.shields.io/badge/cognitive%20eval-57%2F57%20canonical-2EA44F?style=flat-square" alt="cognitive eval">
  <img src="https://img.shields.io/badge/repl%20replay-35%2F35%20canonical-2EA44F?style=flat-square" alt="repl replay">
  ...
  <img src="https://img.shields.io/badge/world%20core-947%20curated%20/%201116%20facts-9CCC65?style=flat-square" alt="world core">
...
Live numbers (verified 2026-04-28 against the actual repo): cognitive eval **57 / 57 canonical, 0 aspirational**. REPL replay: **35 / 35 canonical + 0 aspirational**. World Core: **947 entries / 1116 curated facts across 30 domains** (+73 entries from v4.4.10 Kazakhstan geography expansion). Reasoner: **10 of 11 rules firing** with **22 387 derived facts** over **15 642 extracted + curated facts**. Workspace: **687 tests passing**, 0 warnings. Lexicon: **~25.5 k roots** (13 606 pure Kazakh + 11 919 Apertium imports). Every curated fact carries `ConfidenceKind::HumanApproved` with a named reviewer; every derivation has a `rule_id` + non-empty `source_chain`; every belief fact carries `Provenance` + `FactStatus`; every remembered place carries `EntityMemory.canonical_id`; every dialog turn's lookups are declared as `ToolCall`s and recorded as `ToolResult`s with typed `ToolEvidence` on `TurnTrace.tool_calls`. Nothing ungrounded leaves the deterministic recognised / grounded runtime path.

#### Verified-on-2026-04-27 quick reference

| Claim | Verified value | Verification path |
|---|---|---|
| Workspace tests | **687 passing, 0 failing, 4 ignored** | `cargo test --workspace` |
| Cognitive eval canonical | **57 / 57** | `cargo test -p adam-dialog --test cognitive_eval` |
| REPL replay | **35 / 35 canonical + 0 aspirational** | `cargo test -p adam-dialog --test repl_replay` |
| World Core entries / facts / domains | **947 / 1116 / 30** | `find data/world_core -name '*.jsonl' \| xargs cat \| jq -s 'length'` |
| Extracted runtime facts | **15 642** | `jq '.counts.facts_total' data/retrieval/facts.json` |
| Derived facts | **22 387** | sum of `data/retrieval/derived_facts.json` `.counts.by_rule` values |
...
| Facts (committed runtime) | **15 642 total** = **14 526 extracted (Grammar)** + **1116 curated (HumanApproved, 30 domains)**. T4_200k scale |
| **Rule-derived facts (committed runtime)** | **22 387 derivations** (10 active rules; numeric breakdown in the rules row above) |
...
| Cognitive eval baseline (v4.4.10) | **57 / 57 canonical, 0 aspirational** — every scenario the harness has tracked since v4.0.34 now passes; growth log in `docs/roadmap.md` |
| REPL replay baseline (v4.4.10) | **35 / 35 canonical, 0 aspirational** — v4.4.10 added 4 new dialogs from a 2026-04-28 real-REPL transcript (intro proposal, oblast count, Қысқасы no-misfire, full intro flow); harness now loads runtime artefacts (facts.json + derived_facts.json + morpheme_index.json) so retrieval-dependent dialogs reach the production code path |
...
| Predicates defined | **11** — IsA, LivesIn, Has, GoesTo, PartOf, RelatedTo, Causes, After, HasQuantity, DoesTo, InDomain |
| Extracted / curated / derived facts (committed runtime) | **14 526 extracted + 1116 curated (world_core, 30 domains) / 22 387 derived** (T4_200k text-extraction scale; numeric per-rule breakdown in the Capabilities table) |

...
<p align="center">
  <a href="https://github.com/qazaq-ai/adam/releases"><img src="https://img.shields.io/badge/version-6.19.0-2EA44F?style=for-the-badge" alt="version"></a>
...
- **Camera proctoring (v6.14.0)** — the worker page snaps a webcam photo at session start; its SHA-256 is sealed into the допуск (`idMethod: camera-proctored`, `proctorSha256`), binding *who physically took the briefing* to the signed credential.  The photo lives on the enterprise's server, the hash inside the signature — accident-review evidence that «кто проходил» can't be swapped after the fact.  Consent is handled at hiring.
- **Browser demos + our neural Kazakh voice (v6.15.x)** — one-command launcher [`scripts/demo.sh`](scripts/demo.sh) builds the portal and opens the pitch, general-dialog, worker and ИТР pages.  The AI reads Kazakh with **our own neural voice** via `/api/tts` (Piper `kk_KZ-issai-high`), not the browser's mangled Kazakh; a general-dialog page ([`demo/portal_dialog.html`](demo/portal_dialog.html)) exercises the deterministic engine with voice in/out.
- **Bilingual kz/ru — mandatory for ССГПО (v6.16.0 → v6.17.0)** — briefings and knowledge-checks run in the worker's **chosen language, Kazakh or Russian, end-to-end**: schema `_ru` fields on `ProcedureIR` + a language-parametric `BriefingSession` + **all 33 procedures translated to Russian** (`validate_procedures` = ru 33/33) + a Russian human-readable protocol (`render_ru`).  The signed anchors (`sopHash` + Kazakh-canonical question-type tags) stay **language-invariant**, so a допуск is version-stable and comparable across languages.  >60 % of ССГПО workers don't read Kazakh, so Russian delivery was a hard pilot blocker — now closed.  All four demo pages carry a kz/ru switcher.
- **Voice-derived gender + bounded Russian dialog (v6.17.x)** — the general-dialog page estimates the speaker's pitch (F0, autocorrelation) and greets with the correct Kazakh vocative («Ағай» / «Апай»), also an **anti-impersonation** signal for industrial safety.  Russian turns get a bounded conversational responder ([`lang_bridge::respond_ru`](crates/adam-dialog/src/lang_bridge.rs): greeting, identity, capabilities, arithmetic, capitals) so the demo dialog is visibly bilingual, with an honest Russian capability message for anything outside that scope.

...
<p align="center">
  <a href="https://github.com/qazaq-ai/adam/releases"><img src="https://img.shields.io/badge/version-6.11.0-2EA44F?style=for-the-badge" alt="version"></a>
...
- **Protocol Seal (v6.11.0)** — the finished допуск protocol can be **cryptographically signed** and independently verified.  New zero-dependency [`adam-seal`](crates/adam-seal) crate implements SHA-256 / SHA-512 / Ed25519 (RFC 8032) in pure Rust — the whole signature path is auditable in-tree and gated on the **official RFC 8032 known-answer vectors**.  [`briefing_seal`](crates/adam-dialog/src/briefing_seal.rs) wraps the graded protocol plus caller context (worker / operator / timestamp / site) into a deterministic canonical envelope and signs it: `adam_briefing keygen` mints a key, `--sign-key`/`--seal-out` seal a session, and `adam_briefing verify <sealed.json>` checks the seal and exits non-zero if it does not hold.  Editing any field — e.g. flipping the admission verdict after the fact — breaks the signature, so the protocol is a tamper-**evident** legal artifact, not just a demo transcript.  Not constant-time; scoped to offline on-device sealing.

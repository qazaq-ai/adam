...
- **Forward-chaining reasoner**: 5 active rules (R1 IsA-transitivity, R2 Has-inheritance, R3 Has-via-PartOf, R5 shared-IsA → RelatedTo, R6 LivesIn-via-PartOf, R7 GoesTo-via-PartOf). R4 IsA-symmetry is curator-warning only. R6/R7 refuse astronomical-scale derived targets (v4.0 contradiction filter). Every derivation carries a `rule_id` + non-empty `source_chain` + `ConfidenceKind::RuleInferred`.
...
### Quality gates
- **Full regression test suite (444 workspace tests as of v4.0.1, 0 failing, 0 warnings)** + `scripts/validate_foundation.sh` foundation CI + `scripts/verify_release_version.sh` manifest-consistency gate.

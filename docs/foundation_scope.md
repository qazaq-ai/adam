...
## In Scope (v1.0.0 → v3.0 delivered)
...
- Template repository as external TOML data (34 families as of v3.9.0)
- **World Core v1** (v3.9.0+): human-authored Kazakh knowledge packs in `data/world_core/<domain>.jsonl` (80 entries / 126 curated facts at bootstrap across astronomy / time / geography_kz), emitted with `ConfidenceKind::HumanApproved` — the exclusive tier marking human-reviewed claims
...
- Full regression test suite (433 workspace tests as of v3.9.0, 0 failing, 0 warnings) and foundation CI
...
This is a weaker claim than "whole output is FST-guaranteed" — which would require every literal template token to pass back through the FST at runtime, which the current realiser does not do. Stated accurately for v1.3.x – v3.0.

...
- **World Core** (v3.9.0+): human-authored Kazakh knowledge packs in `data/world_core/<domain>.jsonl` — **507 entries / 601 curated facts as of v4.0.0** across 13 domains (astronomy, time, geography_kz, biology_basic, body_parts, society, colors, numbers, kz_literature, food, clothing, proverbs, animals), emitted with `ConfidenceKind::HumanApproved` — the exclusive tier marking human-reviewed claims.

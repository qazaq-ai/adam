...
- **Precision hardening** (v3.8.5 → v4.0): location allow-list, time-noun block, demonstrative closed-class, possessive-tainted object refusal, central `is_fragment_root` post-filter (`-`-prefixed). **v4.0 contradiction immune system**: `is_astronomical_object` helper blocks R6/R7 derivations against celestial-scale targets (closes `(бала, LivesIn, күн жүйесі)` cross-domain leak); object-side 3-char minimum in locative/dative matchers; 20+ new closed-class entries (conjunctions `егер`/`алайда`/`бірақ`/…, adverbials `жалға`/`тек`/`қана`/`ғана`, fragment-suffix standalones `ған`/`ген`/`ын`/`ін`/…).
- **World Core** (v3.9.0+): human-authored Kazakh knowledge packs in `data/world_core/<domain>.jsonl` — **549 entries / 643 curated facts as of v4.0.7** across 14 domains (astronomy, time, geography_kz, biology_basic, body_parts, society, colors, numbers, kz_literature, food, clothing, proverbs, animals, transport), emitted with `ConfidenceKind::HumanApproved` — the exclusive tier marking human-reviewed claims.
- **Forward-chaining reasoner**: 7 active rules — R1 IsA-transitivity, R2 Has-inheritance, R3 Has-via-PartOf, R5 shared-IsA → RelatedTo, R6 LivesIn-via-PartOf, R7 GoesTo-via-PartOf, **R8 After-transitivity (v4.0.4 — pure temporal order)**. R4 IsA-symmetry is curator-warning only. R6/R7 refuse astronomical-scale derived targets. Every derivation carries a `rule_id` + non-empty `source_chain` + `ConfidenceKind::RuleInferred`.
...
### Quality gates
- **Full regression test suite (463 workspace tests as of v4.0.6, 0 failing, 0 warnings)** + `scripts/validate_foundation.sh` foundation CI + `scripts/verify_release_version.sh` manifest-consistency gate.

...
> **v6.0 forward-looking note (added 2026-05-16).** The deterministic pipeline this document describes (L1 parser → L2 semantics → L3 retrieval → L4 reasoning → L5 template → L6 verifier → L7 audit → L8 surface) is preserved load-for-load in v6.0. v6.0 adds **L5.5 NEURAL COMPOSITION** between template (L5) and verifier (L6), strictly additive: a missing model file boots and runs the v5.x behaviour unchanged. See [`architecture_neural_v6.md`](architecture_neural_v6.md) for the v6.0 specification and [`MANIFESTO.md`](MANIFESTO.md) for the architectural position behind the addition.
...
- **v4.0.0** — **World Core 500+ + contradiction immune system (major)**. Codex's v3.9.5 review surfaced specific false chains in `facts.json`: «бала lives_in күн жүйесі», «(егер, DoesTo, газ)», «(жалға, GoesTo, жер)» — reasoning was scaling faster than precision. v4.0.0 responds on both axes. **World Core**: 200 → 507 entries / 270 → 601 facts with 7 new domains (`colors` 37/38, `numbers` 45/54, `kz_literature` 60/69, `food` 50/50, `clothing` 35/35, `proverbs` 40/43, `animals` 40/42). **Contradiction immune system**: R6/R7 refuse astronomical-scale derived targets (`is_astronomical_object` helper — жер/ай/күн/ғаламшар/күн жүйесі/…); object-side 3-char minimum in locative_lives_in and dative_goes_to closes `-ған/-ын/-ін` fragment-tail cases; `is_closed_class` expansion with 20+ conjunctions + adverbials + fragment-suffix standalones. 3 new R6/R7 guard tests. **Measured at T4_200k**: facts **13 889** (13 288 extracted + 601 curated), derivations **6 579 (×3.2)** with R5 exploding 933 → 5 437 (×5.8) from dense world_core IsA base, R6/R7 astronomical guards blocked 410 false chains. Graph 3 286 nodes / 12 447 edges. 443 tests.

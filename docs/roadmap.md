...
| **v1.3.5** | **Sharding + docs drift + v2.0 direction committed** — `--full` mode writes 10 shards (≤50 MB each) to gitignored `data/curated/shards/` for local retrieval-engine fuel; `corpus_audit --local` scans them. Docs fixed: badge 253→256, `foundation_scope` no longer claims trilingual/transliteration (reverted v1.1.0), "FST-guaranteed" wording tightened to slot path only. **v2.0 committed as retrieval-over-corpus, not a trained LM.** | 256 workspace tests; local corpus now 16 M words, gap to 100 M = 6.2× |

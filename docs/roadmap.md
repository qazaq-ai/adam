...
| **v1.4.0** | **FST-NER + DST + predicate-copula FST** — added `Predicate` feature to `NounFeatures` (7 person variants + 6 suffix templates); +30 Kazakh place-name entries in Lexicon (Алматы, Астана, Шымкент, …); `detect_statement_of_location` / `detect_statement_of_occupation` refactored to use `Analysis` as primary path; `Conversation` gained `active_intent` + `intent_history` + `resolve_follow_up` for "ал сіз?" handling | **262 workspace tests**; closes Codex/Antigravity FST-ignored critique; DST enables multi-turn context |

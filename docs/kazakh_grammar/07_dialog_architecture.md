...
> **Historical context.** This document captures the architecture as it was at the v1.0.0 MVP cut. The v1.1.0 → v3.0 arc has added: retrieval engine (`adam-retrieval`), reasoning bootstrap (`adam-reasoning`) with fact extraction + lexical graph + forward-chaining rule reasoner, composition marker (v1.9.5), reasoning chain marker (v2.7), and FST correctness fixes (v2.3). For the current single source of truth see [`docs/architecture_v3.md`](../architecture_v3.md). The v2.0 retrieval-era snapshot at [`docs/architecture_v2.md`](../architecture_v2.md) remains valid as a historical reference.
